...
def export_latest_csv_sheets(xlsx_path: Path, csv_dir: Path) -> List[Path]:
    """
    Export poster/automation-facing workbook sheets to CSV.

    Why:
      - ChatGPT scheduled tasks and lightweight readers handle GitHub raw CSV
        more reliably than binary .xlsx files.
      - Keep objectives_latest.xlsx as the human-checkable source workbook,
        and use latest_csv/*.csv as automation input.

    Exported sheets:
      - Core sheets needed for poster generation:
        SB_Report, Mission_DB, New_Report_Index, Combo_Report_Index
      - Dynamic report sheets:
        New_Report_XX, Combo_Report_XX
      - Optional detail sheet for debugging/poster verification:
        SB_Action_Detail

    The folder is refreshed on each run so stale New_Report_XX/Combo_Report_XX
    files from previous runs do not remain in GitHub.
    """
    ...
    try:
        manifest = pd.DataFrame({
            "Sheet": target_sheets,
            "CSV_File": [f"{s}.csv" for s in target_sheets],
        })
