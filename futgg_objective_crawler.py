...
KNOWN_LEAGUES = {
    "Bundesliga", "Premier League", "LALIGA EA SPORTS", "LALIGA", "LaLiga", "Serie A", "Ligue 1",
...
def is_objective_detail_url(url: str) -> bool:
    u = canonical_url(url)
    return re.search(r"/objectives/(campaigns|live-events|seasonal|milestones|fc-pro)/[0-9]+-[^/]+/$", u) is not None
...
ACTION_START_RE = re.compile(
    r"^(Win|Play|Score|Assist|Complete|Keep|Earn|Make|Get|Perform|Record|Concede|Claim|Finish|Achieve|Watch|Tune|View|Link)\b",
    re.I,
)
...
def looks_like_description(line: str) -> bool:
    ...
    has_context = bool(re.search(
        r"\b(matches?|goals?|assists?|clean sheets?|Squad Battles|Rivals|Champions|Live Events?|Rush|starting 11|using|with|while having|difficulty|broadcast|stream|FC Pro|Twitch|YouTube|watch)\b",
        t,
        re.I,
    ))
...
def parse_action(desc: str, mission_name: str = "") -> Tuple[str, int, str, str]:
    ...
    # not the number of goals/assists in each match.
    # Examples:
    #   Score a finesse goal in 5 separate matches -> count 5, action Finesse Goal
    #   Assist 1 goal from a cross in 4 separate matches -> count 4, action Cross Assist
    #   Concede 1 goal or less in 5 separate matches -> count 5, action Concede Limit
    separate_patterns = [
        # Concede-limit objectives.
        ("Concede Limit", r"\bConcede\s+(?:1|one)\s+goals?\s+or\s+less\s+in\s+([0-9]+)\s+separate\s+(?:\w+\s+){0,6}matches?\b"),
        ("Concede Limit", r"\bConcede\s+(?:1|one)\s+or\s+less\s+goals?\s+in\s+([0-9]+)\s+separate\s+(?:\w+\s+){0,6}matches?\b"),
        ("Concede Limit", r"\bConcede\s+no\s+more\s+than\s+(?:1|one)\s+goals?\s+in\s+([0-9]+)\s+separate\s+(?:\w+\s+){0,6}matches?\b"),

        # Assist/special-assist objectives. Keep these before generic Assist count patterns.
        # Examples:
        #   Assist with a through ball in 25 separate matches -> Through Ball Assist, count 25
        #   Assist 1 goal from a cross in 4 separate matches -> Cross Assist, count 4
        ("Assist", r"\bAssist\s+with\s+(?:a\s+)?(?:through\s+ball|lobbed\s+through\s+ball|incisive\s+pass|inventive\s+pass|flair\s+pass)\s+in\s+([0-9]+)\s+separate\s+(?:\w+\s+){0,6}matches?\b"),
        ("Assist", r"\bAssist(?:\s+at\s+least)?\s+(?:1|one)\s+goals?\s+from\s+a\s+cross\s+in\s+([0-9]+)\s+separate\s+(?:\w+\s+){0,6}matches?\b"),
        ("Assist", r"\bAssist(?:\s+at\s+least)?\s+(?:1|one)\s+goals?\s+in\s+([0-9]+)\s+separate\s+(?:\w+\s+){0,6}matches?\b"),
        ("Assist", r"\bAssist\s+in\s+([0-9]+)\s+separate\s+(?:\w+\s+){0,6}matches?\b"),

        # Goal/finesse/header/special goal objectives.
        # Examples:
        #   Score with a header in 3 separate matches -> Header Goal, count 3
        #   Score a finesse goal in 5 separate matches -> Finesse Goal, count 5
        ("Goal", r"\bScore\s+(?:with|using)\s+(?:a\s+)?(?:header|headed\s+goal|volley|power\s+shot|chip\s+shot|low\s+driven\s+shot)\s+in\s+([0-9]+)\s+separate\s+(?:\w+\s+){0,6}matches?\b"),
        ("Goal", r"\bScore\s+(?:with|using)\s+(?:a\s+)?finesse\s+shot\s+in\s+([0-9]+)\s+separate\s+(?:\w+\s+){0,6}matches?\b"),
        ("Goal", r"\bScore\s+(?:a\s+)?finesse\s+goals?\s+in\s+([0-9]+)\s+separate\s+(?:\w+\s+){0,6}matches?\b"),
        ("Goal", r"\bScore(?:\s+at\s+least)?\s+(?:1|one|a)\s+goals?\s+in\s+([0-9]+)\s+separate\s+(?:\w+\s+){0,6}matches?\b"),
        ("Goal", r"\bScore\s+(?:in\s+)?([0-9]+)\s+separate\s+(?:\w+\s+){0,6}matches?\b"),
    ]
    ...
    patterns = [
        ...
        ("Concede Limit", r"\bConcede\s+(?:1|one)\s+goals?\s+or\s+less\b"),
        ("Concede Limit", r"\bConcede\s+(?:1|one)\s+or\s+less\s+goals?\b"),
    ]
...
def parse_performer_conditions(desc: str, mission_id: str, action: str, count: int, per_match: str) -> List[Dict]:
    ...
    # Explicit position + attribute catches.
    # Example: using a Striker from LALIGA -> Value=LALIGA, Position=ST
    position_aliases = {
        "striker": "ST",
        "forward": "ST",
        "defender": "Defender",
    ...
    for m_pos in re.finditer(
        r"\busing\s+(?:a|an)\s+(Striker|Forward|Defender|Midfielder|Attacker|Goalkeeper|GK|LB|LWB|CB|RB|RWB|CDM|CM|CAM|LM|RM|LW|RW|ST|CF)\s+from\s+(?:the\s+)?(.+?)(?:\.|$)",
        d,
        re.I,
    ):
        pos_raw = clean_text(m_pos.group(1))
        val = normalize_value(strip_trailing_context(m_pos.group(2)))
        pos = position_aliases.get(pos_raw.lower(), pos_raw.upper() if len(pos_raw) <= 3 else pos_raw)
        if val:
            found_with_pos.append((val, pos))
    ...
    if not found and not found_with_pos and (
    ...
    for val, pos in found_with_pos:
        ...
        rows.append({
            ...
            "Condition_Type": "Performer",
            "Target": action,
            "Attribute": classify_value(val),
            "Value": val,
            "Count": count,
            "Position": pos,
            "Per_Match": per_match,
            "Operational_Note": performer_note(action, count, per_match, val, pos),
        })

    return dedupe_condition_rows(rows)


def performer_note(action: str, count: int, per_match: str, val: str, position: str = "") -> str:
    label = clean_text(f"{val} {position}") if position else val
    if per_match == "Yes":
        return f"{label}: {count}경기 각각 {action} 1회 이상"
    return f"{label}: {action} {count}회"



def extract_formation(desc: str) -> str:
    """
    Extract tactic/formation wording such as:
      using 4-4-2
      using 4-1-2-1-2 (2)
    """
    m = re.search(r"\busing\s+([0-9](?:-[0-9]){1,6}(?:\s*\([0-9]+\))?)(?=\s|\.|,|$)", clean_text(desc), re.I)
    return clean_text(m.group(1)) if m else ""


def action_operation_note(action: str, count: int, per_match: str = "No") -> str:
    """Korean compact instruction for non-player-specific action/result conditions."""
    try:
        c = int(count or 0)
    except Exception:
        c = 0
    ...
    if action == "Clean Sheet":
        return f"클린시트 {c}회"
    if action == "Concede Limit":
        if per_match == "Yes":
            return f"{c}경기 각각 1실점 이하"
        return "1실점 이하"
    return f"{action} {c}" if c else action


def formation_operation_note(formation: str, action: str, count: int, per_match: str = "No") -> str:
    """Formation objectives should remain visible, but should not replace goal/assist rows."""
    if not formation:
        return ""
    if action in {"Win", "Play", "Complete", "Clean Sheet", "Concede Limit"}:
        return f"{formation} 전술로 {action_operation_note(action, count, per_match)}"
    return f"{formation} 전술 사용"
...
def parse_result_conditions(desc: str, mission_id: str, action: str, count: int, per_match: str, requires_win: str) -> List[Dict]:
    ...
    elif action == "Concede Limit":
        rows.append({
            "Mission_ID": mission_id,
            "Condition_Type": "Other_Action",
            "Target": "Concede Limit",
            "Attribute": "Any",
            "Value": "Any",
            "Count": count,
            "Position": "",
            "Per_Match": per_match,
            "Operational_Note": action_operation_note(action, count, per_match),
        })
...
def parse_mission_conditions(mission: Dict) -> Tuple[Dict, List[Dict]]:
    ...
    # Formation/tactic requirements such as "using 4-4-2" are gameplay conditions.
    # Important display rule:
    #   - If the mission is a Goal/Assist objective, the formation is part of that action condition.
    #     Show it in Goals/Assists, not as a separate To do(기타) line.
    #     Example: Score a finesse goal ... using 4-1-2-1-2 (2)
    #       -> Goals: 4-1-2-1-2 (2) 전술: 5경기 각 Finesse Goal 1회
    #   - If the mission is a Win/Play/Concede objective, show the formation in Other_Actions
    #     as a contextual phrase such as "5-4-1 전술로 5승".
    formation = extract_formation(desc)
    if formation:
        if is_goal_target(action) or is_assist_target(action):
            # Remove generic Any performer rows for this same action, because the formation
            # is the actual condition attached to the goal/assist objective.
            conditions = [
                c for c in conditions
                if not (
                    c.get("Condition_Type") == "Performer"
                    and c.get("Target") == action
                    and c.get("Value") == "Any"
                )
            ]

            # If a concrete performer already exists, combine it with the formation so the
            # report does not imply alternative ways. Otherwise, use the formation itself
            # as the action qualifier.
            attached = False
            for c in conditions:
                if c.get("Condition_Type") == "Performer" and c.get("Target") == action:
                    val = clean_text(str(c.get("Value", "")))
                    pos = clean_text(str(c.get("Position", "")))
                    base = clean_text(f"{val} {pos}") if pos else val
                    if base and formation not in base:
                        c["Value"] = f"{base} + {formation} 전술"
                        c["Position"] = ""
                        c["Attribute"] = "Combined"
                        c["Operational_Note"] = performer_note(action, count, per_match, c["Value"])
                    attached = True
            ...
            if not attached:
                conditions.append({
                    ...
                    "Target": action,
                    "Attribute": "Formation",
                    "Value": f"{formation} 전술",
                    "Count": count,
                    "Position": "",
                    "Per_Match": per_match,
                    "Operational_Note": performer_note(action, count, per_match, f"{formation} 전술"),
                })
        else:
            conditions.append({
                "Mission_ID": mission["Mission_ID"],
                ...
                "Position": "",
                "Per_Match": per_match,
                "Operational_Note": formation_operation_note(formation, action, count, per_match),
            })
...
def compact_performer_requirements(cond_rows: List[Dict], target_filter: Optional[str] = None) -> str:
    ...
    for r in rows:
        val = r["Value"]
        pos = clean_text(str(r.get("Position", "") or ""))
        label = clean_text(f"{val} {pos}") if pos else val
        target = r["Target"]
        cnt = int(r.get("Count") or 0)
        if r.get("Per_Match") == "Yes":
            lines.append(f"{label}: {cnt}경기 각 {target} 1회")
        else:
            lines.append(f"{label}: {target} {cnt}회")
...
def compact_other_actions(cond_rows: List[Dict]) -> str:
    ...
    # If a formation-specific objective exists and there is no squad condition,
    # suppress generic duplicates such as both '5승' and '5-4-1 전술로 5승'.
    has_formation = any(r.get("Condition_Type") == "Other_Action" and r.get("Target") == "Formation" for r in cond_rows)
    has_squad = any(r.get("Condition_Type") == "Squad" for r in cond_rows)
    if not (has_formation and not has_squad):
    ...
    for r in cond_rows:
        ...
        if r["Condition_Type"] == "Other_Action":
            ...
            # When a formation-specific version exists, avoid duplicating generic
            # Concede Limit rows such as both '5경기 각각 1실점 이하' and
            # '5-3-2 전술로 5경기 각각 1실점 이하'.
            if has_formation and not has_squad and r.get("Target") == "Concede Limit":
                continue
...
def compact_for_mission(cond_rows: List[Dict], mission_row: Dict) -> Dict:
    ...
    performer_vals = []
    for c in cond_rows:
        # Formation-only qualifiers are action conditions, not bench/player candidates.
        if c.get("Condition_Type") == "Performer" and c.get("Value") and c.get("Value") != "Any" and c.get("Attribute") != "Formation":
            pos = clean_text(str(c.get("Position", "") or ""))
            performer_vals.append(clean_text(f"{c.get('Value')} {pos}") if pos else c.get("Value"))
...
def main():
    ...
    if EXCLUDE_SEASONAL_FROM_MAIN_PLAN and not df_mission.empty:
        # Main gameplay plan excludes Seasonal and FC Pro/watch-link missions.
        # Mission_DB still keeps them for source tracking, but SB_Report/Action_Plan should
        # focus on playable objectives. Milestone objectives such as Get in the Game remain included.
        non_seasonal_ids = set(df_mission.loc[
            ~df_mission["Objective_Type"].isin(["Seasonal", "FC Pro"]),
            "Mission_ID"
        ])
