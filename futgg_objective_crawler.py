...
#
# v28: set fixed daily 02:00 KST as deadline calculation base and keep per-mode New_Report sheets.
#
# Purpose:
    ...
    for _, row in df.iterrows():
        ...
        if has_value:
            ...
            if display and display.lower() != "nan":
                rows.append(f"{campaign} ({display})")
            else:
...
def build_mode_report_rows(
    ...
    """
    발표자료/포스터 제작 전 단계의 간단 보고서용 시트.
    기본 대상: Squad Battles / Rivals / Champions / Live Events

    only_new=True이면 해당 모드의 신규 미션(Is_New=Yes)만 모아 같은 형식으로 요약합니다.

    3열 구조:
      - 구분: 보고서용 항목명
      - 내용: 핵심 요약
      - 관련 캠페인: 포스터 하단/작은 글씨로 활용 가능한 근거 캠페인
    """
    ...
    rows = [
        ...
        },
        {
            "구분": "최소 필요경기",
            "내용": f"{min_matches}경기",
            "관련 캠페인": campaigns_for_column(df, ["Results", "Goals", "Assists", "Other_Actions"]),
        },
        {
            "구분": "플레이 요건",
            "내용": result_summary_for_report(df),
            "관련 캠페인": campaigns_for_column(df, ["Results"]),
        },
        {
            "구분": "필수 선발 요건",
            "내용": unique_join_starting_with_new(df),
            "관련 캠페인": campaigns_for_column(df, ["Starting_XI"]),
        },
        {
            "구분": "교체로 투입해도 가능",
            "내용": performer_candidates_excluding_starting_with_new(df),
            "관련 캠페인": campaigns_for_column(df, ["Performer_Candidates"]),
        },
        {
            "구분": "To do (득점)",
            "내용": unique_join_from_column_with_new(df, "Goals"),
            "관련 캠페인": campaigns_for_column(df, ["Goals"]),
        },
        {
            "구분": "To do (어시스트)",
            "내용": unique_join_from_column_with_new(df, "Assists"),
            "관련 캠페인": campaigns_for_column(df, ["Assists"]),
        },
        {
            "구분": "To do (기타)",
            "내용": unique_join_from_column_with_new(df, "Other_Actions"),
            "관련 캠페인": campaigns_for_column(df, ["Other_Actions"]),
        },
    ]
    return pd.DataFrame(rows)
...
def write_excel(out_path: Path, df_mission: pd.DataFrame, df_conditions: pd.DataFrame, df_plan: pd.DataFrame,
    ...
    with pd.ExcelWriter(out_path, engine="openpyxl") as writer:
        ...
        guide = pd.DataFrame([
            {"Item": "가장 먼저 볼 시트", "Description": "SB_Report: 대상 모드 기준 핵심 수행 요약입니다. C열 관련 캠페인은 포스터 하단/작은 글씨용 근거로 활용할 수 있습니다."},
