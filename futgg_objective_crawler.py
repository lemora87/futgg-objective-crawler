...
def main():
    ...
    if EXCLUDE_SEASONAL_FROM_MAIN_PLAN and not df_mission.empty:
        # 기본 실행계획에서는 장기 Seasonal과 방송시청성 FC Pro를 제외합니다.
        # 단, Mission_DB/Condition_DB에는 남겨 원문 수집 여부를 확인할 수 있게 합니다.
        exclude_from_main_types = {"Seasonal", "FC Pro"}
        main_ids = set(df_mission.loc[~df_mission["Objective_Type"].isin(exclude_from_main_types), "Mission_ID"])
        seasonal_ids = set(df_mission.loc[df_mission["Objective_Type"] == "Seasonal", "Mission_ID"])

        df_mission_main = df_mission[df_mission["Mission_ID"].isin(main_ids)].copy()
        df_conditions_main = df_conditions[df_conditions["Mission_ID"].isin(main_ids)].copy()
