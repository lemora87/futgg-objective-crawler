...
#
# v32: normalize FUT.GG relative expiry deadlines to the usual 02:00 KST deadline time.
# v31: use domcontentloaded instead of networkidle to avoid FUT.GG/GitHub Actions hanging.
# v30: also export SB_Report/New_Report CSV sidecar files for GitHub Raw access.
#
# Purpose:
...
def expiry_deadline_kst(expires_text: str, base_dt: Optional[datetime] = None) -> tuple[str, str]:
    """
    Convert relative FUT.GG wording into an estimated Korean-time deadline.

    v32 rule:
      Most FUT.GG objectives appear to expire at 02:00 KST.
      Therefore, use the relative "Expires in ..." text mainly to estimate the
      expiry DATE, then normalize the final time to 02:00 KST.

    Why:
      If we literally add "Expires in 17 hours" to a 02:00 base, the display can
      become 19:00 or 04:00, even though the actual objective reset/deadline is
      usually 02:00 KST. This is especially noisy when GitHub Actions starts late.

    Returns:
      (deadline_display, deadline_iso)
      Example: ('5/10 02:00', '2026-05-10T02:00+09:00')
    """
    ...
    if base_dt is None:
        # Stable daily 02:00 KST base to avoid GitHub Actions delay noise.
    ...
    # First estimate the rough date using the relative duration.
    # For day-based text, the 02:00 base is intentionally used.
    # For hour/minute-only text, using base_dt still keeps the workflow stable;
    # then we normalize the result back to 02:00 on the estimated date.
    estimated = base_dt + timedelta(days=days, hours=hours, minutes=minutes)

    # Normalize to the usual FUT objective deadline time: 02:00 KST.
    deadline = estimated.replace(
        hour=DEADLINE_BASE_HOUR_KST,
        minute=DEADLINE_BASE_MINUTE_KST,
        second=0,
        microsecond=0,
    ...
    # If the relative duration includes hours/minutes and the estimated timestamp
    # passes 02:00 on the next calendar date, keep that estimated date.
    # Example: base 5/8 02:00 + 26 hours -> estimated 5/9 04:00 -> 5/9 02:00.
    # Example: base 5/8 02:00 + 17 hours -> estimated 5/8 19:00 -> 5/8 02:00.
    # This matches the rule "date from relative text, time fixed at 02:00".
...
def collect_objective_links(headless: bool = True) -> List[Dict]:
    with sync_playwright() as p:
        ...
        # Avoid networkidle hangs; give client-side FUT.GG content a short fixed render wait.
        time.sleep(2)
...
def compact_other_actions(cond_rows: List[Dict]) -> str:
    ...
    - Clean Sheet는 Results보다 Other_Actions에서 보는 편이 직관적이라 여기로 보냅니다.
    - Goal/Assist 계열은 세부 표현이 있더라도 Goals/Assists로 보냅니다.
      예: Outside Box Goal -> Goals, Through Ball Assist -> Assists.
    """
    lines = []
    ...
    for r in cond_rows:
        ...
        if r["Condition_Type"] == "Match_Result" and r["Target"] == "Clean Sheet":
            lines.append(f"Clean Sheet {r['Count']}")
...
# SB_Report는 실제로 스쿼드 배틀을 돌 때 같이 클리어 가능한 미션을 모으는 용도입니다.
# 따라서 "Any Ultimate Team mode" 계열 미션도 SB_Report에 함께 포함합니다.
# 단, New_Report_XX처럼 특정 신규 모드만 보여주는 보고서에는 섞지 않습니다.
ANY_ULTIMATE_TEAM_MODE_KEYWORD = "Any Ultimate Team mode"


def is_sb_report_compatible_mode(mode_group: str, target_mode: str = TARGET_REPORT_MODE) -> bool:
    """
    Return True for rows that should appear in the default SB_Report.

    FUT.GG의 "Any Ultimate Team mode" 조건은 Squad Battles에서도 수행 가능하므로
    기본 SB_Report에 포함해야 합니다. 예: Squad Battles를 플레이하면서 동시에 클리어 가능.
    "Live Events / Any Ultimate Team mode"처럼 복합 표기도 포함합니다.
    """
    mode = clean_text(str(mode_group or ""))
    if mode == target_mode:
        return True
    if ANY_ULTIMATE_TEAM_MODE_KEYWORD.lower() in mode.lower():
        return True
    return False


def filter_report_mode_rows(
    ...
    """
    Report-row filter.
    - 기본 SB_Report: target mode + Any Ultimate Team mode 계열 포함
    - New_Report_XX 등 only_new=True: 해당 target_mode만 정확히 필터링
    """
    if df_action is None or df_action.empty or "Mode_Group" not in df_action.columns:
        return pd.DataFrame()

    if target_mode == TARGET_REPORT_MODE and not only_new:
        mask = df_action["Mode_Group"].astype(str).map(lambda x: is_sb_report_compatible_mode(x, target_mode))
        return df_action[mask].copy()

    return df_action[df_action["Mode_Group"].astype(str) == str(target_mode)].copy()
...
def build_mode_report_rows(
    ...
    new_sheet_map = build_new_sheet_lookup(df_action)
    ...
    df = filter_report_mode_rows(df_action, target_mode=target_mode, only_new=only_new)
    ...
    rows = [
        {"구분": report_label, "내용": target_mode, "관련 캠페인": "-", "신규여부": "N", "신규시트": ""},
        {
            "구분": "최소 필요경기",
            "내용": f"{min_matches}경기",
            "관련 캠페인": "-",
    ...
    append_report_section(rows, "플레이 요건", report_result_rows(df, new_sheet_map), empty_text="-")
    append_report_section(rows, "필수 선발 요건", report_starting_rows(df, new_sheet_map))
    append_report_section(rows, "교체로 투입해도 가능", report_performer_candidate_rows(df, new_sheet_map))
    append_report_section(rows, "To do (득점)", report_lines_for_column(df, "Goals", new_sheet_map))
    append_report_section(rows, "To do (어시스트)", report_lines_for_column(df, "Assists", new_sheet_map))
    append_report_section(rows, "To do (기타)", report_lines_for_column(df, "Other_Actions", new_sheet_map))
...
def build_mode_action_detail(df_action: pd.DataFrame, target_mode: str = TARGET_REPORT_MODE) -> pd.DataFrame:
    ...
    df = filter_report_mode_rows(df_action, target_mode=target_mode, only_new=False)
    if df.empty:
        return pd.DataFrame()
...
def csv_safe_filename(name: str) -> str:
    """
    Make a simple safe filename for GitHub Raw CSV sidecar files.
    Keep names stable so links such as latest/SB_Report.csv remain usable.
    """
    s = clean_text(str(name or "report"))
    s = re.sub(r"[^A-Za-z0-9_.-]+", "_", s)
    s = re.sub(r"_+", "_", s).strip("_.")
    return s or "report"


def write_csv_sidecars(out_dir: Path, df_action: Optional[pd.DataFrame]) -> Dict[str, Path]:
    """
    Write poster/report sheets as standalone CSV files.

    Why:
      GitHub Raw .xlsx files can be awkward for lightweight readers, but CSV files
      are plain text and can be opened directly from raw.githubusercontent.com.

    Output examples:
      SB_Report.csv
      New_Report_Index.csv
      New_Report_01.csv
      New_Report_02.csv
      SB_Action_Detail.csv
      Action_Plan.csv
    """
    ...
    def save_csv(df: pd.DataFrame, filename: str):
        path = out_dir / csv_safe_filename(filename)
        # utf-8-sig keeps Korean text readable in Excel while remaining valid CSV for GitHub Raw.
    ...
    # 기본 SB report
    sb_report = build_mode_report_rows(df_action)
    save_csv(sb_report, "SB_Report.csv")

    sb_detail = build_mode_action_detail(df_action)
    if sb_detail is not None and not sb_detail.empty:
        save_csv(sb_detail, "SB_Action_Detail.csv")

    save_csv(df_action, "Action_Plan.csv")

    # 신규미션 모드별 CSV. Excel의 New_Report_XX 시트 번호와 같은 규칙을 사용합니다.
    new_sheet_map: Dict[str, str] = {}
    new_csv_map: Dict[str, str] = {}
    if "Is_New" in df_action.columns:
        new_df = df_action[df_action["Is_New"].astype(str).str.lower().eq("yes")].copy()
        if not new_df.empty:
            for idx, mode in enumerate(sorted(new_df["Mode_Group"].dropna().astype(str).unique()), start=1):
                sheet_name = f"New_Report_{idx:02d}"
                csv_name = f"{sheet_name}.csv"
                new_sheet_map[str(mode)] = sheet_name
                new_csv_map[str(mode)] = csv_name
                new_report = build_mode_report_rows(df_action, target_mode=str(mode), only_new=True)
                save_csv(new_report, csv_name)

    index_df = build_new_report_index(df_action, new_sheet_map)
    if "CSV_File" not in index_df.columns:
        insert_at = 1 if "Sheet" in index_df.columns else 0
        csv_files = []
        for _, row in index_df.iterrows():
            mode = str(row.get("Mode_Group", ""))
            csv_files.append(new_csv_map.get(mode, "-"))
        index_df.insert(insert_at, "CSV_File", csv_files)
    save_csv(index_df, "New_Report_Index.csv")
...
def write_excel(out_path: Path, df_mission: pd.DataFrame, df_conditions: pd.DataFrame, df_plan: pd.DataFrame,
    ...
    with pd.ExcelWriter(out_path, engine="openpyxl") as writer:
            ...
            if "Is_New" in df_action.columns:
                ...
                if not new_df.empty:
                    for idx, mode in enumerate(sorted(new_df["Mode_Group"].dropna().astype(str).unique()), start=1):
                        ...
                        build_mode_report_rows(df_action, target_mode=mode, only_new=True).to_excel(
                            writer,
                            sheet_name=sheet_name,
                            index=False,
                        )

            build_new_report_index(df_action, new_sheet_map).to_excel(writer, sheet_name="New_Report_Index", index=False)
        ...
        guide = pd.DataFrame([
            ...
            {"Item": "New_Report_Index", "Description": "신규미션이 있는 모드별 New_Report 시트를 안내합니다."},
            ...
            {"Item": "신규미션 표시", "Description": "latest/objectives_latest.xlsx의 Mission_DB와 비교해 새로 추가된 미션은 SB_Report/New_Report의 신규여부=Y로 표시합니다. 선발/교체 요건은 신규로 생겼거나 요구 수량이 증가한 경우에만 Y로 표시합니다."},
...
def format_workbook(path: Path):
    ...
    widths = {
        "SB_Report": {"A": 24, "B": 60, "C": 58, "D": 10, "E": 18},
        "New_Report_Index": {"A": 18, "B": 22, "C": 46, "D": 18, "E": 12, "F": 48},
        "SB_Action_Detail": {
    ...
    for ws in wb.worksheets:
        ...
        if ws.title == "SB_Report" or ws.title.startswith("New_Report_"):
            for row in ws.iter_rows(min_row=2, max_row=max_row, max_col=max_col):
                for cell in row:
...
def is_new_value(value) -> bool:
    return str(value or "").strip().lower() == "yes"


def build_new_sheet_lookup(df_action: pd.DataFrame) -> Dict[str, str]:
    """
    Return the same mode -> New_Report_XX mapping used by Excel/CSV exports.
    This lets SB_Report carry a stable 신규시트 value for poster automation.
    """
    if df_action is None or df_action.empty or "Is_New" not in df_action.columns or "Mode_Group" not in df_action.columns:
        return {}
    ...
    return {str(mode): f"New_Report_{idx:02d}" for idx, mode in enumerate(sorted(new_df["Mode_Group"].dropna().astype(str).unique()), start=1)}


def report_new_fields(is_new: bool, mode_group: str = "", new_sheet_map: Optional[Dict[str, str]] = None) -> Dict[str, str]:
    """Poster-friendly new mission columns for SB_Report/New_Report CSV."""
    new_sheet_map = new_sheet_map or {}
    return {
        "신규여부": "Y" if is_new else "N",
        "신규시트": new_sheet_map.get(str(mode_group), "") if is_new else "",
    }


def report_lines_for_column(df: pd.DataFrame, col: str, new_sheet_map: Optional[Dict[str, str]] = None) -> List[Dict]:
    """
    Return one report row per visible item so B열 내용 and C열 관련 캠페인이 1:1 대응됩니다.
    신규여부/신규시트 열을 함께 내려 포스터 자동화에서 NEW 배지를 붙일 수 있게 합니다.
    """
    rows = []
    if df is None or df.empty or col not in df.columns:
        return rows
    ...
    seen = set()
    for _, row in df.iterrows():
        is_new = is_new_value(row.get("Is_New", "No"))
        campaign = campaign_display_from_row(row)
        mode_group = str(row.get("Mode_Group", ""))
        for line in split_cell_lines(row.get(col, "")):
            text = re.sub(r"\s*\(신규미션\)\s*$", "", line).strip()
            item = {
                "내용": text,
                "관련 캠페인": campaign,
                **report_new_fields(is_new, mode_group, new_sheet_map),
            }
            key = (item["내용"], item["관련 캠페인"], item["신규여부"], item["신규시트"])
            if key not in seen:
                seen.add(key)
                rows.append(item)
    return rows
...
def report_starting_rows(df: pd.DataFrame, new_sheet_map: Optional[Dict[str, str]] = None) -> List[Dict]:
    """
    Starting_XI rows with conservative new/increased marker and 1:1 campaign mapping.
    신규로 생겼거나 요구 수량이 증가한 경우 신규여부=Y로 표시합니다.
    """
    if df is None or df.empty or "Starting_XI" not in df.columns:
        return []
    ...
    for idx, row in df.iterrows():
        is_new = is_new_value(row.get("Is_New", "No"))
        for line in split_cell_lines(row.get("Starting_XI", "")):
            line = re.sub(r"\s*\(신규미션\)\s*$", "", line).strip()
            parsed = parse_ge_requirement_line(line)
            if not parsed:
                key = ("RAW", line)
                label = line
    ...
    for key in sorted(all_max.keys(), key=lambda x: str(x)):
        ...
        for _, row, _, label in candidates:
            ...
            item = {
                "내용": text,
                "관련 캠페인": campaign,
                **report_new_fields(is_new_or_increased, mode_group, new_sheet_map),
            }
            dedupe = (item["내용"], item["관련 캠페인"], item["신규여부"], item["신규시트"])
...
def report_performer_candidate_rows(df: pd.DataFrame, new_sheet_map: Optional[Dict[str, str]] = None) -> List[Dict]:
    """
    ...
    신규 행에서 처음 등장한 후보만 신규여부=Y로 표시합니다.
    """
    if df is None or df.empty:
        return []
    ...
    if "Performer_Candidates" in df.columns:
        for _, row in df.iterrows():
            is_new = is_new_value(row.get("Is_New", "No"))
            vals = [re.sub(r"\s*\(신규미션\)\s*$", "", v).strip() for v in split_cell_lines(row.get("Performer_Candidates", ""))]
            vals = [v for v in vals if v not in starting_values]
            if not is_new:
    ...
    if "Performer_Candidates" in df.columns:
        for _, row in df.iterrows():
            ...
            for v in split_cell_lines(row.get("Performer_Candidates", "")):
                text = re.sub(r"\s*\(신규미션\)\s*$", "", v).strip()
                if text in starting_values:
                    continue
                item_is_new = is_new and text not in old_vals
                item = {
                    "내용": text,
                    "관련 캠페인": campaign,
                    **report_new_fields(item_is_new, mode_group, new_sheet_map),
                }
                key = (item["내용"], item["관련 캠페인"], item["신규여부"], item["신규시트"])
                if key not in seen:
                    seen.add(key)
...
def report_result_rows(df: pd.DataFrame, new_sheet_map: Optional[Dict[str, str]] = None) -> List[Dict]:
    """Consolidated play/result requirements with 신규여부/신규시트 columns."""
    if df is None or df.empty or "Results" not in df.columns:
        return []

    out = []
    result_lines = consolidate_result_lines(df["Results"].dropna().astype(str).tolist())
    for line in result_lines:
        contributors = df[df["Results"].astype(str).str.contains(re.escape(line), na=False)].copy()
        if contributors.empty:
            contributors = df.copy()
        is_new = contributors["Is_New"].astype(str).str.lower().eq("yes").any() if "Is_New" in contributors.columns else False
        # If multiple new modes somehow contribute, show the first stable New_Report_XX value.
        new_sheet = ""
        if is_new and "Mode_Group" in contributors.columns:
            for mode in contributors["Mode_Group"].dropna().astype(str).tolist():
                if mode in (new_sheet_map or {}):
                    new_sheet = (new_sheet_map or {}).get(mode, "")
                    break
        out.append({
            "내용": line,
            "관련 캠페인": campaigns_for_column(contributors, ["Results"]),
            "신규여부": "Y" if is_new else "N",
            "신규시트": new_sheet if is_new else "",
        })
    return out
...
def append_report_section(rows: List[Dict], section: str, items: List[Dict], empty_text: str = "-"):
    ...
    신규여부/신규시트는 포스터 자동화를 위한 열입니다.
    """
    if not items:
        rows.append({"구분": section, "내용": empty_text, "관련 캠페인": "-", "신규여부": "N", "신규시트": ""})
        return
...
def main():
    ...
    csv_files = write_csv_sidecars(out_dir, df_action)

    print()
    print(f"Done: {out_path}")
    if csv_files:
        print("CSV sidecar files written for GitHub Raw access:")
