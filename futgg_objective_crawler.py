...
def parse_action(desc: str, mission_name: str = "") -> Tuple[str, int, str, str]:
    ...
    separate_patterns = [
        ...
        ("Concede Limit", r"\bConcede\s+(?:1|one)\s+goals?\s+or\s+less\s+in\s+([0-9]+)\s+separate\s+(?:\w+\s+){0,5}matches?\b"),
        ("Concede Limit", r"\bConcede\s+(?:1|one)\s+or\s+less\s+goals?\s+in\s+([0-9]+)\s+separate\s+(?:\w+\s+){0,5}matches?\b"),
        ("Concede Limit", r"\bConcede\s+no\s+more\s+than\s+(?:1|one)\s+goals?\s+in\s+([0-9]+)\s+separate\s+(?:\w+\s+){0,5}matches?\b"),

        # Assist/cross objectives. Keep this before generic Assist count patterns.
        ("Assist", r"\bAssist(?:\s+at\s+least)?\s+(?:1|one)\s+goals?\s+(?:from\s+a\s+cross\s+)?in\s+([0-9]+)\s+separate\s+(?:\w+\s+){0,5}matches?\b"),
        ("Assist", r"\bAssist\s+in\s+([0-9]+)\s+separate\s+(?:\w+\s+){0,5}matches?\b"),

        # Goal/finesse/special goal objectives.
        ("Goal", r"\bScore\s+(?:with\s+)?(?:a\s+)?(?:finesse\s+shot|finesse\s+goal)\s+in\s+([0-9]+)\s+separate\s+(?:\w+\s+){0,5}matches?\b"),
        ("Goal", r"\bScore\s+using\s+(?:a\s+)?finesse\s+shot\s+in\s+([0-9]+)\s+separate\s+(?:\w+\s+){0,5}matches?\b"),
        ("Goal", r"\bScore(?:\s+at\s+least)?\s+(?:1|one|a)\s+goals?\s+in\s+([0-9]+)\s+separate\s+(?:\w+\s+){0,5}matches?\b"),
        ("Goal", r"\bScore\s+(?:in\s+)?([0-9]+)\s+separate\s+(?:\w+\s+){0,5}matches?\b"),
    ]
    ...
    for action, pat in patterns:
        ...
        if m:
            action = detect_special_action(d, action)
            # Concede-limit simple fallback has no count in the match. Keep existing count 1.
            return action, int(m.group(1)) if m.groups() else 1, per_match, requires_win
...
def formation_operation_note(formation: str, action: str, count: int, per_match: str = "No") -> str:
    """
    Formation objectives should not be hidden. Use a contextual phrase where possible.
    """
    if not formation:
        return ""
...
def should_show_raw_in_other_actions(desc: str, conditions: List[Dict]) -> bool:
    ...
    suspicious_patterns = [
        ...
        r"concede\s+(?:less|fewer)\s+than", r"concede\s+(?:1|one)\s+goals?\s+or\s+less",
        r"concede\s+(?:1|one)\s+or\s+less\s+goals?", r"score\s+and\s+assist",
...
def parse_mission_conditions(mission: Dict) -> Tuple[Dict, List[Dict]]:
    ...
    # Generic fallback: if an action-looking objective is still empty, keep it visible.
    # This mainly covers non-match objectives such as Rush point bonuses or SBC completion.
    if not conditions and ACTION_START_RE.search(desc):
        conditions.append({
            "Mission_ID": mission["Mission_ID"],
            "Condition_Type": "Other_Action",
            "Target": "Raw",
            ...
            "Position": "",
            "Per_Match": per_match,
            "Operational_Note": "미분류 원문 확인: " + desc,
        })
