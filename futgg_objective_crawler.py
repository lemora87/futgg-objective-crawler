...
#
# v22: compare with previous objectives_latest.xlsx and mark new-mission To do lines.
#
# Purpose:
...
def build_mode_report_rows(df_action: pd.DataFrame, target_mode: str = TARGET_REPORT_MODE) -> pd.DataFrame:
    ...
    rows = [
        ...
        {"구분": "To do (득점)", "내용": unique_join_from_column_with_new(df, "Goals")},
        {"구분": "To do (어시스트)", "내용": unique_join_from_column_with_new(df, "Assists")},
        {"구분": "To do (기타)", "내용": unique_join_from_column_with_new(df, "Other_Actions")},
...
def build_mode_action_detail(df_action: pd.DataFrame, target_mode: str = TARGET_REPORT_MODE) -> pd.DataFrame:
    ...
    keep_cols = [
        "Completed", "Is_New", "Campaign", "Mission_Name", "Starting_XI", "Performer_Candidates",
...
def write_excel(out_path: Path, df_mission: pd.DataFrame, df_conditions: pd.DataFrame, df_plan: pd.DataFrame,
    ...
    with pd.ExcelWriter(out_path, engine="openpyxl") as writer:
        ...
        guide = pd.DataFrame([
            ...
            {"Item": "신규미션 표시", "Description": "latest/objectives_latest.xlsx의 Mission_DB와 비교해 새로 추가된 미션의 To do 항목에는 (신규미션)을 붙입니다."},
