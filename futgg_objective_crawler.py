...
#
# v20: portable output paths; default output folder is the script folder, with --out/config overrides.
#
# Purpose:
...
def compact_other_actions(cond_rows: List[Dict]) -> str:
    ...
    for r in cond_rows:
        ...
        if r["Condition_Type"] == "Performer" and r["Target"] not in {"Goal", "Assist"}:
            lines.append(r["Operational_Note"])
...
def build_mode_report_rows(df_action: pd.DataFrame, target_mode: str = TARGET_REPORT_MODE) -> pd.DataFrame:
    """
    ...
    if df.empty:
        return pd.DataFrame({
            "구분": ["대상 모드", "요약"],
            "내용": [target_mode, "해당 모드에서 수행할 미션이 없습니다."]
        })

    min_matches = int(df["Min_Matches"].max()) if "Min_Matches" in df.columns and not df.empty else ""
    campaigns = "\n".join(unique_list(df["Campaign"].dropna().astype(str).tolist()))

    rows = [
        {"구분": "대상 모드", "내용": target_mode},
        {"구분": "최소 필요경기", "내용": f"{min_matches}경기"},
        {"구분": "플레이 요건", "내용": result_summary_for_report(df)},
        {"구분": "필수 선발 요건", "내용": unique_join_from_column_consolidated(df, "Starting_XI", consolidate_ge=True)},
        {"구분": "교체로 투입해도 가능", "내용": performer_candidates_excluding_starting(df)},
        {"구분": "To do (득점)", "내용": unique_join_from_column(df, "Goals")},
        {"구분": "To do (어시스트)", "내용": unique_join_from_column(df, "Assists")},
        {"구분": "To do (기타)", "내용": unique_join_from_column(df, "Other_Actions")},
        {"구분": "대상 캠페인", "내용": campaigns if campaigns else "-"},
    ]
    return pd.DataFrame(rows)
...
def build_mode_action_detail(df_action: pd.DataFrame, target_mode: str = TARGET_REPORT_MODE) -> pd.DataFrame:
    ...
    keep_cols = [
        "Completed", "Campaign", "Mission_Name", "Starting_XI", "Performer_Candidates",
...
def write_excel(out_path: Path, df_mission: pd.DataFrame, df_conditions: pd.DataFrame, df_plan: pd.DataFrame,
    ...
    with pd.ExcelWriter(out_path, engine="openpyxl") as writer:
        ...
        guide = pd.DataFrame([
            {"Item": "가장 먼저 볼 시트", "Description": "SB_Report: 대상 모드 기준 핵심 수행 요약입니다. 2열 구조로 간단히 표시합니다."},
            {"Item": "SB_Action_Detail", "Description": "대상 모드의 세부목표별 근거표입니다. 완료 체크와 상세 확인용입니다."},
            {"Item": "Action_Plan", "Description": "세부목표 1개당 1행으로 Goals/Assists/Other_Actions/Results를 분리해서 보여줍니다."},
            ...
            {"Item": "Other_Actions", "Description": "Clean Sheet, Outside Box Goal, Finesse Goal 등 Goal/Assist 외 특수 수행조건을 표시합니다. 특수조건 파싱이 애매하면 원문을 그대로 표시합니다."},
...
def format_workbook(path: Path):
    ...
    widths = {
        ...
        "Mission_DB": {
            "A": 14, "B": 28, "C": 16, "D": 22, "E": 60, "F": 38, "G": 18,
            "H": 13, "I": 13, "J": 13, "K": 16, "L": 28, "M": 48, "N": 18, "O": 30
        },
        "Condition_DB": {
