...
def _safe_excel_value(value):
    """Return a clean Python value for writing into openpyxl cells."""
    if value is None:
        return None
    try:
        if pd.isna(value):
...
def _plus_value_for_stat(row: pd.Series, stat: str):
    """Return the increase value for the stat shown in the current status column.

    Example:
      - If the status column header is PAC, this returns the evolution's '+ PAC'.
      - If the status column header is Finishing, this returns '+ Finishing'.

    This is intentionally NOT a total sum of every stat increase.
    """
    raw = row.get(f"+ {stat}", None)
...
def _choose_top_evolution_for_stat(df_matrix: pd.DataFrame, stat: str, rank: int):
    """Choose the representative evolution for a stat column and rank.

    Ranking is based on distinct Max values for the stat:
      1위 = highest Max {stat}
      2위 = second-highest distinct Max {stat}
      3위 = third-highest distinct Max {stat}

    The '+Stat Point' shown for that column is the selected evolution's own
    '+ {stat}' value, e.g. '+ PAC' under PAC or '+ Finishing' under Finishing.
    """
    max_col = f"Max {stat}"
    if df_matrix is None or df_matrix.empty or max_col not in df_matrix.columns:
        return None
    ...
    plus_col = f"+ {stat}"
    if plus_col in candidates.columns:
        candidates["__plus_for_stat"] = pd.to_numeric(candidates[plus_col], errors="coerce").fillna(-1)
    else:
        candidates["__plus_for_stat"] = -1
    ...
    candidates = candidates.sort_values(
        by=["__plus_for_stat", "__ovr", "__name"],
    ...
    return {
        ...
        "plus_for_stat": _safe_excel_value(_plus_value_for_stat(row, stat)),
...
def _find_status_sheet_columns(ws):
    ...
    if label_col is None:
        # Most existing analysis files use H=rank and I=label, so J starts stat columns.
        label_col = 9
    if rank_col is None:
    ...
    if not stat_cols:
        # Fallback: status sheet usually starts with PAC, not OVR.
...
def update_status_sheet_layout(wb, df_matrix: pd.DataFrame):
    """Update only '(현황표)' with top-3 blocks.

    Per rank block:
      MAX          = selected Max {stat}
      +OVR         = selected evolution's '+ OVR'
      +Stat Point  = selected evolution's '+ {current column stat}'
                    e.g. PAC column -> '+ PAC', Finishing column -> '+ Finishing'
      진화명       = selected Evolution_Name

    Values are written directly to avoid #N/A formulas. Other sheets are untouched.
    """
    ...
    # Unmerge only the top summary area to rebuild clean 4-row rank blocks.
    ...
    # Clear the rebuilt top summary block only. Header row and lower content remain intact.
    ...
    # Keep stat headers readable and consistent.
    ...
    for rank in range(1, 4):
        ...
        # Compact numeric rows, taller evolution-name row.
        ws.row_dimensions[base_row].height = 20
        ws.row_dimensions[base_row + 1].height = 20
        ws.row_dimensions[base_row + 2].height = 20
        ws.row_dimensions[base_row + 3].height = 46

    # Borders and block separators.
    for row in range(2, 14):
        for col in range(target_min_col, target_max_col + 1):
