...
def parse_upgrades(text: str):
    ...
    for m in pat.finditer(upgrade_text):
        ...
        cap = m.group(3)
        cap_value = int(cap) if cap and cap.isdigit() else None

        # FUT.GG detail pages often repeat the same upgrade text once in the
        # compact summary and again in the detailed UPGRADES block.  Counting
        # those duplicate snippets twice makes +OVR / +Stat values look like
        # 34, 60, etc.  Keep repeated level upgrades, but ignore exact duplicate
        # (stat, increase, cap) snippets from the duplicated page text.
        dedup_key = (stat, inc, cap_value)
        if dedup_key in seen_upgrade_keys:
            continue
        seen_upgrade_keys.add(dedup_key)
...
def _format_status_value(value):
    """Return a clean Excel-friendly display value for the status sheet."""
...
def _join_status_values(values):
    """Join multiple matching evolution values while keeping display compact."""
    out = []
    seen = set()
    for value in values:
        formatted = _format_status_value(value)
...
def update_status_sheet_summary(wb, df_matrix):
    """
    '(현황표)' 상단 요약표를 크롤링 결과 df_matrix 값으로 직접 갱신합니다.

    구조:
      1위: 2~5행 = MAX / +OVR / +Stat Point / 진화명
      2위: 6~9행 = MAX / +OVR / +Stat Point / 진화명
      3위: 10~13행 = MAX / +OVR / +Stat Point / 진화명

    '+Stat Point'는 전체 합산값이 아니라, 각 열 스탯의 증가치입니다.
      예: PAC 열 -> '+ PAC', Finishing 열 -> '+ Finishing'
    """
    sheet_name = "(현황표)"
    if sheet_name not in wb.sheetnames or df_matrix is None or df_matrix.empty:
        return
    ...
    # Determine the stat columns to update from the existing header row.
    stat_columns = []
    for col_idx in range(start_col, ws.max_column + 1):
        stat = clean_text(str(ws.cell(row=1, column=col_idx).value or ""))
        if not stat:
            break
        if f"Max {stat}" in df_matrix.columns:
            stat_columns.append((col_idx, stat))

    if not stat_columns:
        return

    end_col = stat_columns[-1][0]

    # Preserve representative styles from the existing template before edits.
    rank_style_cell = ws.cell(row=2, column=rank_col)
    label_style_cell = ws.cell(row=2, column=label_col)
    value_style_cell = ws.cell(row=2, column=start_col)
    text_style_cell = ws.cell(row=3, column=start_col)

    # Remove old rank merges in H2:H13 so the new 4-row rank blocks can be merged.
    for merged_range in list(ws.merged_cells.ranges):
        if (
            merged_range.min_col <= rank_col <= merged_range.max_col
            and merged_range.min_row <= end_row
            and merged_range.max_row >= start_row
        ):
            ws.unmerge_cells(str(merged_range))

    # Clear only the top summary range values. Styles are kept and then normalized.
    for row_idx in range(start_row, end_row + 1):
        ws.row_dimensions[row_idx].hidden = False
        for col_idx in range(rank_col, end_col + 1):
            ws.cell(row=row_idx, column=col_idx).value = None

    labels = ["MAX", "+OVR", "+Stat Point", "진화명"]
    rank_names = ["1위", "2위", "3위"]

    # Rebuild left rank/label columns.
    for rank_idx, rank_name in enumerate(rank_names):
        base_row = start_row + rank_idx * 4
        ws.merge_cells(start_row=base_row, start_column=rank_col, end_row=base_row + 3, end_column=rank_col)
        rank_cell = ws.cell(row=base_row, column=rank_col, value=rank_name)
        _copy_cell_style(rank_style_cell, rank_cell)
        rank_cell.alignment = Alignment(horizontal="center", vertical="center")
        for offset, label in enumerate(labels):
            label_cell = ws.cell(row=base_row + offset, column=label_col, value=label)
            _copy_cell_style(label_style_cell, label_cell)
            label_cell.alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)

    # Basic borders / fills matching the existing sheet tone.
    thin = Side(style="thin", color="808080")
    medium = Side(style="medium", color="000000")
    label_fill = PatternFill("solid", fgColor="D9EAF7")
    rank_fill = PatternFill("solid", fgColor="9DC3E6")

    for row_idx in range(start_row, end_row + 1):
        ws.row_dimensions[row_idx].hidden = False
        for col_idx in range(rank_col, end_col + 1):
            cell = ws.cell(row=row_idx, column=col_idx)
            bottom_side = medium if row_idx in (5, 9, 13) else thin
            cell.border = Border(left=thin, right=thin, top=thin, bottom=bottom_side)
    ...
    # Keep row heights compact for numbers and larger for evolution names.
    for rank_idx in range(3):
        base_row = start_row + rank_idx * 4
        ws.row_dimensions[base_row].height = 18
        ws.row_dimensions[base_row + 1].height = 18
        ws.row_dimensions[base_row + 2].height = 18
        ws.row_dimensions[base_row + 3].height = 48
        for r in range(base_row, base_row + 4):
            ws.row_dimensions[r].hidden = False

    name_col = "Evolution_Name"
    ovr_col = "+ OVR"

    for col_idx, stat in stat_columns:
        max_col = f"Max {stat}"
        plus_col = f"+ {stat}"

        max_values = pd.to_numeric(df_matrix[max_col], errors="coerce")
        valid_max_values = max_values[(max_values > 0)].dropna()

        if valid_max_values.empty:
            continue

        ranked_max_values = sorted(valid_max_values.unique(), reverse=True)[:3]

        for rank_idx, max_value in enumerate(ranked_max_values):
            base_row = start_row + rank_idx * 4
            mask = max_values == max_value
            matched_rows = df_matrix.loc[mask].copy()

            ws.cell(row=base_row, column=col_idx).value = _format_status_value(max_value)
            ws.cell(row=base_row + 1, column=col_idx).value = _join_status_values(matched_rows[ovr_col]) if ovr_col in matched_rows else None
            ws.cell(row=base_row + 2, column=col_idx).value = _join_status_values(matched_rows[plus_col]) if plus_col in matched_rows else None
            ws.cell(row=base_row + 3, column=col_idx).value = _join_status_values(matched_rows[name_col]) if name_col in matched_rows else None

            # Numeric rows stay centered; evolution name row can be small/wrapped.
            for r in (base_row, base_row + 1, base_row + 2):
                cell = ws.cell(row=r, column=col_idx)
                _copy_cell_style(value_style_cell, cell)
                cell.alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)
                cell.border = Border(left=thin, right=thin, top=thin, bottom=thin)

            name_cell = ws.cell(row=base_row + 3, column=col_idx)
            _copy_cell_style(text_style_cell, name_cell)
            name_cell.alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)
            name_cell.border = Border(left=thin, right=thin, top=thin, bottom=medium)

    # Rows 14~26 are remnants of the old hidden helper block. Clear and hide them so they do not appear as stale data.
    cleanup_start, cleanup_end = 14, 26
    clear_end_col = max(end_col, min(ws.max_column, 46))
    for merged_range in list(ws.merged_cells.ranges):
        if (
            merged_range.min_col <= clear_end_col
            and merged_range.max_col >= rank_col
            and merged_range.min_row <= cleanup_end
            and merged_range.max_row >= cleanup_start
        ):
            ws.unmerge_cells(str(merged_range))
    for row_idx in range(cleanup_start, cleanup_end + 1):
        ws.row_dimensions[row_idx].hidden = True
        for col_idx in range(rank_col, clear_end_col + 1):
            ws.cell(row=row_idx, column=col_idx).value = None

    # Refresh the optional detailed helper table from row 27 onward.
    # It is kept hidden by default, but if the user unhides it, it reflects the latest df_matrix.
    update_status_detail_table(ws, df_matrix)

    # Ensure workbook opens on the summary sheet and recalculates dependent formulas elsewhere if needed.
...
def update_status_detail_table(ws, df_matrix):
    """
    Refresh the optional detail table at rows 27+.
    기준 스탯은 J9 값이 있으면 J9, 없으면 J1(PAC 등 첫 스탯)을 사용합니다.
    기존 수식 대신 현재 df_matrix 값으로 직접 채웁니다.
    """
    ...
    selected_stat = clean_text(str(ws.cell(row=9, column=start_col).value or ""))
    if not selected_stat or f"Max {selected_stat}" not in df_matrix.columns:
        selected_stat = clean_text(str(ws.cell(row=1, column=start_col).value or ""))
    if not selected_stat or f"Max {selected_stat}" not in df_matrix.columns:
        return

    max_col = f"Max {selected_stat}"
    plus_col = f"+ {selected_stat}"

    # Reset headers to a clear, current layout.
    headers = [
        "선택한 스탯의 Max값", "진화명", "Rank", "+OVR",
        "Max OVR", "포지션 제한", "OVR 제한", "기타 요구조건"
    ]
    for idx, header in enumerate(headers, start=start_col):
        ws.cell(row=start_row, column=idx).value = header

    # Clear old formulas/values in the helper table area.
    for row_idx in range(data_start, data_start + max_rows):
        for col_idx in range(start_col, end_col + 1):
            ws.cell(row=row_idx, column=col_idx).value = None

    tmp = df_matrix.copy()
    tmp["_selected_max"] = pd.to_numeric(tmp[max_col], errors="coerce")
    tmp = tmp[tmp["_selected_max"].notna() & (tmp["_selected_max"] > 0)].copy()
    if tmp.empty:
        return

    tmp["_plus_stat"] = pd.to_numeric(tmp[plus_col], errors="coerce") if plus_col in tmp.columns else None
    tmp["_plus_ovr"] = pd.to_numeric(tmp["+ OVR"], errors="coerce") if "+ OVR" in tmp.columns else None
    tmp["_max_ovr"] = pd.to_numeric(tmp["Max OVR"], errors="coerce") if "Max OVR" in tmp.columns else None
    tmp = tmp.sort_values(
        by=["_selected_max", "_plus_stat", "_plus_ovr", "Evolution_Name"],
        ascending=[False, False, False, True],
        na_position="last",
    )

    unique_max = sorted(tmp["_selected_max"].dropna().unique(), reverse=True)
    rank_map = {v: i + 1 for i, v in enumerate(unique_max)}

    # Fill all current rows up to max_rows so the table can be unhidden and used for drill-down.
    out_rows = []
    for _, row in tmp.head(max_rows).iterrows():
        max_val = row.get("_selected_max")
        req_min = row.get("Req_OVR_Min") if "Req_OVR_Min" in tmp.columns else None
        req_max = row.get("Req_OVR_Max") if "Req_OVR_Max" in tmp.columns else None
        ovr_limit_parts = []
        if not _is_blank_value(req_min):
            ovr_limit_parts.append(f"Min {req_min}")
        if not _is_blank_value(req_max):
            ovr_limit_parts.append(f"Max {req_max}")
        ovr_limit = " / ".join(ovr_limit_parts) if ovr_limit_parts else None

        other = row.get("Req_Other") if "Req_Other" in tmp.columns else row.get("Raw_Requirements_Text")
        out_rows.append([
            _format_status_value(max_val),
            row.get("Evolution_Name"),
            rank_map.get(max_val),
            _format_status_value(row.get("_plus_ovr")),
            _format_status_value(row.get("_max_ovr")),
            row.get("Req_Position") if "Req_Position" in tmp.columns else None,
            ovr_limit,
            other,
        ])

    for r_offset, out_row in enumerate(out_rows):
        row_idx = data_start + r_offset
        for c_offset, value in enumerate(out_row):
            ws.cell(row=row_idx, column=start_col + c_offset).value = value

    # Keep the helper table hidden by default, preserving the original workbook behavior.
    # If unhidden manually, it now contains current values, not stale formulas.
    for row_idx in range(start_row, data_start + max_rows):
        ws.row_dimensions[row_idx].hidden = True
...
def update_analysis_workbook(df_matrix, analysis_path):
    ...
    # '(현황표)' 상단 요약표도 현재 크롤링 결과 기준으로 함께 갱신합니다.
    update_status_sheet_summary(wb, df_matrix)
