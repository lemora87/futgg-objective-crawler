...
def _cell_ref(row: int, col: int, abs_row: bool = False, abs_col: bool = False) -> str:
    """Return an A1-style reference for formulas."""
    col_letter = get_column_letter(col)
    return f"{'$' if abs_col else ''}{col_letter}{'$' if abs_row else ''}{row}"


def _first_row_last_col(ws, min_col: int = 1) -> int:
    """Find the last non-empty header cell in row 1."""
    last_col = min_col
...
def update_status_sheet_layout(wb):
    """
    '(현황표)' 시트를 상위 3개 순위 요약표로 재구성합니다.

    각 스탯 열은 다음 4행 블록으로 구성됩니다.
      1위: MAX / +OVR / +Stat Point / 진화명
      2위: MAX / +OVR / +Stat Point / 진화명
      3위: MAX / +OVR / +Stat Point / 진화명

    중요:
    - +Stat Point는 합산값이 아니라 해당 열 스탯의 증가치입니다.
      예: PAC 열은 Evo[+ PAC], Finishing 열은 Evo[+ Finishing]
    - +OVR은 Evo[+ OVR] 값을 표시합니다.
    - 모든 조회식은 Evo 표를 기준으로 하므로, Unified_Matrix의 Evo 표 갱신 후 호출해야 합니다.
    """
    sheet_name = "(현황표)"
    ...
    # 기존 양식을 최대한 보존하기 위해 1행의 '구분' 열을 기준으로 스탯 시작 열을 찾습니다.
    rank_col = None
    label_col = None
    for col in range(1, ws.max_column + 1):
        value = ws.cell(row=1, column=col).value
        text = str(value).replace("\n", " ").strip() if value is not None else ""
        if label_col is None and text == "구분":
            label_col = col
        if rank_col is None and ("MAX수치" in text or "순위" in text):
            rank_col = col

    if label_col is None:
        # 기존 템플릿 기준 H=순위, I=구분, J부터 스탯입니다.
        label_col = 9
    if rank_col is None:
        rank_col = max(1, label_col - 1)

    start_col = label_col + 1
    end_col = _first_row_last_col(ws, min_col=start_col)
    if end_col < start_col:
        return

    # 기존 병합이 있으면 1~3위 4행 블록으로 다시 병합할 수 있도록 해제합니다.
    for merged in list(ws.merged_cells.ranges):
        if (merged.min_row <= 27 and merged.max_row >= 2 and
                merged.min_col <= label_col and merged.max_col >= rank_col):
            ws.unmerge_cells(str(merged))

    # 3위까지만 보이도록 기존 2~27행의 값/수식을 정리합니다.
    for row in range(2, 28):
        for col in range(rank_col, end_col + 1):
            ws.cell(row=row, column=col).value = None

    # 보기 좋은 기본 서식
    blue_fill = PatternFill("solid", fgColor="9DC3E6")
    light_fill = PatternFill("solid", fgColor="DDEBF7")
    white_font = Font(color="FFFFFF", bold=True)
    bold_font = Font(bold=True)
    thin = Side(style="thin", color="808080")
    medium = Side(style="medium", color="000000")
    thin_border = Border(left=thin, right=thin, top=thin, bottom=thin)
    top_medium_border = Border(left=thin, right=thin, top=medium, bottom=thin)

    # 좌측 헤더 재정리
    ws.cell(row=1, column=rank_col, value="MAX수치\n순위")
    ws.cell(row=1, column=label_col, value="구분")
    for col in (rank_col, label_col):
        cell = ws.cell(row=1, column=col)
        cell.fill = PatternFill("solid", fgColor="4F81BD")
        cell.font = white_font
        cell.alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)

    # 전체 스탯 열 헤더도 기존 파란 헤더 양식을 유지/보정합니다.
    for col in range(start_col, end_col + 1):
        ...
        ws.column_dimensions[get_column_letter(col)].width = max(ws.column_dimensions[get_column_letter(col)].width or 0, 14)

    # 순위별 4행 블록 작성
    for rank in range(1, 4):
        base_row = 2 + (rank - 1) * 4
        prev_max_ref = _cell_ref(base_row - 4, start_col) if rank > 1 else None

        ws.merge_cells(start_row=base_row, start_column=rank_col, end_row=base_row + 3, end_column=rank_col)
        rank_cell = ws.cell(row=base_row, column=rank_col, value=f"{rank}위")
        rank_cell.fill = blue_fill
        ...
        for col in range(start_col, end_col + 1):
            stat_header_ref = _cell_ref(1, col)  # 예: J1, S1
            max_cell_ref = _cell_ref(base_row, col)
            prev_cell_ref = _cell_ref(base_row - 4, col) if rank > 1 else None
            max_table_ref = f'INDIRECT("Evo[Max "&{stat_header_ref}&"]")'
            evo_row_idx = (
                'AGGREGATE(15,6,'
                '(ROW(Evo[Evolution_Name])-MIN(ROW(Evo[Evolution_Name]))+1)/'
                f'(({max_table_ref}={max_cell_ref})*({max_table_ref}>0)),1)'
            )

            if rank == 1:
                max_formula = f'=IFERROR(MAXIFS({max_table_ref},{max_table_ref},">0"),"")'
            else:
                max_formula = f'=IFERROR(MAXIFS({max_table_ref},{max_table_ref},">0",{max_table_ref},"<"&{prev_cell_ref}),"")'

            # 해당 MAX를 만든 첫 번째 진화의 +OVR / 해당 스탯 증가치 / 진화명을 가져옵니다.
            ovr_formula = f'=IFERROR(INDEX(INDIRECT("Evo[+ OVR]"),{evo_row_idx}),"")'
            stat_point_formula = f'=IFERROR(INDEX(INDIRECT("Evo[+ "&{stat_header_ref}&"]"),{evo_row_idx}),"")'
            name_formula = f'=IFERROR(INDEX(Evo[Evolution_Name],{evo_row_idx}),"")'

            ws.cell(row=base_row, column=col, value=max_formula)
            ws.cell(row=base_row + 1, column=col, value=ovr_formula)
            ws.cell(row=base_row + 2, column=col, value=stat_point_formula)
            ws.cell(row=base_row + 3, column=col, value=name_formula)

            for r in range(base_row, base_row + 4):
                cell = ws.cell(row=r, column=col)
                cell.alignment = Alignment(horizontal="center", vertical="center", wrap_text=(r == base_row + 3))
                cell.border = top_medium_border if r == base_row else thin_border
                if r == base_row + 3:
                    cell.alignment = Alignment(horizontal="left", vertical="center", wrap_text=True)

    # 행 높이: 진화명 행은 텍스트가 길므로 조금 크게, 숫자 행은 압축
    for row in range(2, 14):
        ws.row_dimensions[row].height = 48 if row in (5, 9, 13) else 18
    ws.row_dimensions[1].height = 24
    ws.column_dimensions[get_column_letter(rank_col)].width = 9
    ws.column_dimensions[get_column_letter(label_col)].width = 13

    # Excel에서 열었을 때 수식 재계산을 유도합니다.
    try:
...
def update_analysis_workbook(df_matrix, analysis_path):
    ...
    # 현황표 요약 레이아웃 갱신: MAX / +OVR / 해당 스탯 증가치 / 진화명
