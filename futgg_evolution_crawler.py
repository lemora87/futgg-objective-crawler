...
SPECIAL_STATS = ["SM", "WF"]
# FUT.GG upgrade text may include an aggregate value such as "+91 Stat Point".
# It is not a player stat column, but we keep it separately for the analysis summary sheet.
EXTRA_UPGRADE_STATS = ["Stat Point"]
...
STAT_PATTERNS = sorted(
    set(FACE_STATS + SPECIAL_STATS + SUB_STATS + EXTRA_UPGRADE_STATS + list(STAT_ALIASES.keys())),
    key=len,
    reverse=True,
)
...
def matrix_base_row():
    ...
    for stat in ALL_STATS:
        row[f"+ {stat}"] = None
        row[f"Max {stat}"] = None
    # Aggregate upgrade value parsed from strings such as "+91 Stat Point".
    # There is no corresponding Max column for this value.
    row["+ Stat Point"] = None
    return row
...
def merge_upgrade_to_matrix(row, upgrade_rows):
    for u in upgrade_rows:
        ...
        if stat == "Stat Point":
            inc = u.get("Increase")
            if inc is not None:
                row["+ Stat Point"] = (row.get("+ Stat Point") or 0) + inc
            continue
...
def _stat_point_value(row: pd.Series):
    """Return + Stat Point when parsed, otherwise estimate from individual + stat columns."""
    val = row.get("+ Stat Point", None)
    if not pd.isna(val):
        return _safe_excel_value(val)

    # Fallback for older crawls before + Stat Point parsing was added.
    # Sum face/sub-stat upgrades only; OVR, SM and WF are not stat-point totals.
    total = 0
    has_any = False
    excluded = {"+ OVR", "+ SM", "+ WF", "+ Stat Point"}
    for col, raw in row.items():
        if not isinstance(col, str) or not col.startswith("+ ") or col in excluded:
            continue
...
def _choose_top_evolution_for_stat(df_matrix: pd.DataFrame, stat: str, rank: int):
    """
    For a status-sheet stat header (PAC, SHO, Acceleration, ...), choose one
    representative evolution for the rank-th distinct Max value.
    """
    max_col = f"Max {stat}"
    if df_matrix.empty or max_col not in df_matrix.columns:
        return None

    work = df_matrix.copy()
    work["__max"] = pd.to_numeric(work[max_col], errors="coerce")
    work = work[work["__max"].notna() & (work["__max"] > 0)].copy()
    if work.empty:
        return None

    ranked_max_values = sorted(work["__max"].unique(), reverse=True)
    if len(ranked_max_values) < rank:
        return None

    target_max = ranked_max_values[rank - 1]
    candidates = work[work["__max"] == target_max].copy()
    if candidates.empty:
        return None

    if "+ OVR" in candidates.columns:
        candidates["__ovr"] = pd.to_numeric(candidates["+ OVR"], errors="coerce").fillna(-1)
    else:
        candidates["__ovr"] = -1
    candidates["__stat_point"] = candidates.apply(lambda r: _stat_point_value(r), axis=1)
    candidates["__stat_point_sort"] = pd.to_numeric(candidates["__stat_point"], errors="coerce").fillna(-1)
    if "Evolution_Name" in candidates.columns:
        candidates["__name"] = candidates["Evolution_Name"].fillna("").astype(str)
    else:
        candidates["__name"] = ""
    candidates = candidates.sort_values(
        by=["__stat_point_sort", "__ovr", "__name"],
    ...
    return {
        "max": _safe_excel_value(row["__max"]),
        "ovr": _safe_excel_value(row.get("+ OVR", None)),
        "stat_point": _safe_excel_value(row.get("__stat_point", None)),
        "name": _safe_excel_value(row.get("Evolution_Name", None)),
    }


def _find_status_sheet_columns(ws):
    """Find rank/label/stat columns from the existing (현황표) header row."""
    label_col = None
    rank_col = None
    for cell in ws[1]:
        value = clean_text(str(cell.value)) if cell.value is not None else ""
        if value == "구분":
    ...
    if label_col is None:
        # Fallback to the layout used in the user's analysis workbook screenshots.
        label_col = 9  # I
    if rank_col is None:
        rank_col = max(1, label_col - 1)

    stat_cols = []
    valid_stats = set(ALL_STATS)
    for cell in ws[1]:
        if cell.column <= label_col:
            continue
        value = clean_text(str(cell.value)) if cell.value is not None else ""
        if value in valid_stats:
            stat_cols.append((cell.column, value))

    # If the header row is incomplete, fall back to the known status-sheet order.
    if not stat_cols:
        start_col = label_col + 1
        stat_cols = [(start_col + idx, stat) for idx, stat in enumerate(FACE_STATS[1:] + SUB_STATS)]

    return rank_col, label_col, stat_cols


def update_status_sheet_layout(wb, df_matrix: pd.DataFrame):
    """
    Update only the '(현황표)' sheet with a compact top-3 max summary.

    Layout per stat column:
      1위: row 2 MAX, row 3 +OVR, row 4 +Stat Point, row 5 진화명
      2위: row 6 MAX, row 7 +OVR, row 8 +Stat Point, row 9 진화명
      3위: row 10 MAX, row 11 +OVR, row 12 +Stat Point, row 13 진화명

    Values are written directly, not as formulas, so #N/A does not remain.
    Other sheets are not touched.
    """
    if "(현황표)" not in wb.sheetnames or df_matrix is None or df_matrix.empty:
        return

    ws = wb["(현황표)"]
    rank_col, label_col, stat_cols = _find_status_sheet_columns(ws)
    last_data_col = stat_cols[-1][0]
    labels = ["MAX", "+OVR", "+Stat Point", "진화명"]

    # Remove old merged regions in the top summary area only.
    target_min_col = min(rank_col, label_col)
    target_max_col = max(last_data_col, label_col)
    for merged in list(ws.merged_cells.ranges):
        if merged.max_row >= 2 and merged.min_row <= 13 and merged.max_col >= target_min_col and merged.min_col <= target_max_col:
            ws.unmerge_cells(str(merged))

    # Clear only the top summary area, preserving headers and lower analysis rows.
    ...
    name_fill = PatternFill("solid", fgColor="FFFFFF")
    thin = Side(style="thin", color="808080")
    medium = Side(style="medium", color="000000")

    for col, stat in stat_cols:
    ...
    for rank in range(1, 4):
        ...
        rank_cell = ws.cell(row=base_row, column=rank_col)
        rank_cell.value = f"{rank}위"
        rank_cell.fill = rank_fill
        rank_cell.font = Font(bold=True, color="1F4E79")
        rank_cell.alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)

        for offset, label in enumerate(labels):
            cell = ws.cell(row=base_row + offset, column=label_col)
            cell.value = label
        ...
        for col, stat in stat_cols:
            top = _choose_top_evolution_for_stat(df_matrix, stat, rank)
            values = [None, None, None, None] if top is None else [
                top["max"],
                top["ovr"],
                top["stat_point"],
                top["name"],
            ...
            for offset, value in enumerate(values):
                ...
                cell.alignment = Alignment(
                    horizontal="center" if offset < 3 else "left",
                    vertical="center",
                    wrap_text=True,
                    shrink_to_fit=(offset < 3),
                )
                if offset == 3:
                    cell.fill = name_fill
                    cell.font = Font(size=9)

        # Row heights: compact numeric rows, taller evolution-name row.
        ws.row_dimensions[base_row].height = 20
        ws.row_dimensions[base_row + 1].height = 20
        ws.row_dimensions[base_row + 2].height = 20
        ws.row_dimensions[base_row + 3].height = 42

    # Borders for the whole status block. Thick horizontal borders separate ranks.
    for row in range(2, 14):
        for col in range(target_min_col, target_max_col + 1):
            top_side = medium if row in (2, 6, 10) else thin
            bottom_side = medium if row in (5, 9, 13) else thin
            ws.cell(row=row, column=col).border = Border(left=thin, right=thin, top=top_side, bottom=bottom_side)

    # Keep the label area readable and avoid extremely narrow stat columns.
    ws.column_dimensions[get_column_letter(rank_col)].width = max(ws.column_dimensions[get_column_letter(rank_col)].width or 0, 8)
    ws.column_dimensions[get_column_letter(label_col)].width = max(ws.column_dimensions[get_column_letter(label_col)].width or 0, 13)
...
def main():
    ...
    if "+ Stat Point" in df_matrix.columns:
        stat_cols.insert(1, "+ Stat Point")
