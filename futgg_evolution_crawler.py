...
#
# Run:
#   py C:\Users\user\futgg_evo_crawler_fixed_v6.py --show
# or:
#   py C:\Users\user\futgg_evo_crawler_fixed_v6.py
...
OUT_DIR = Path(r"C:\Users\user\내 드라이브(xhdzhogksgksrl@gmail.com)\(개인폴더)\에볼루션 크롤링")

# 기존 분석용 엑셀 파일명입니다.
# 실제 분석 파일명이 다르면 아래 파일명만 바꾸세요.
ANALYSIS_FILE = OUT_DIR / "futgg_evolutions_analysis.xlsx"

# True: ANALYSIS_FILE의 Unified_Matrix/Evo 표를 자동 갱신
# False: 크롤링 결과 파일만 생성
UPDATE_ANALYSIS_FILE = True
...
def get_all_link_rows(headless: bool = True) -> List[Dict]:
    with sync_playwright() as p:
        ...
        page_current.goto(START_URL, wait_until="networkidle", timeout=60000)
        time.sleep(2)
        ...
        page_reward.goto(START_URL, wait_until="networkidle", timeout=60000)
        time.sleep(2)
...
def main():
    ...
    out_path = OUT_DIR / f"futgg_evolutions_unified_{today}.xlsx"
    ...
    if UPDATE_ANALYSIS_FILE:
        if ANALYSIS_FILE.exists():
            ...
            print("분석파일명을 ANALYSIS_FILE 변수에 맞게 바꾸거나, 해당 파일을 OUT_DIR 폴더에 넣어주세요.")
    ...
    summary_rows += [
        ...
        {"Metric": "Rule", "Value": "v6 uses early working detail parser; only link collection/source grouping changed."},
